...
struct TextPostProcessorTests {
    ...
    func lightITNFunctionOfAndSubscripts() {
        ...
        // Linear algebra: transpose / inverse / dagger / trace / rank
        #expect(TextPostProcessor.process("transpose of A") == "Aᵀ")
        #expect(TextPostProcessor.process("the transpose of B") == "Bᵀ")
        #expect(TextPostProcessor.process("A transpose") == "Aᵀ")
        #expect(TextPostProcessor.process("A transposed") == "Aᵀ")
        #expect(TextPostProcessor.process("inverse of A") == "A⁻¹")
        #expect(TextPostProcessor.process("the inverse of M") == "M⁻¹")
        #expect(TextPostProcessor.process("A inverse") == "A⁻¹")
        #expect(TextPostProcessor.process("A dagger") == "A†")
        #expect(TextPostProcessor.process("hermitian of A") == "A†")
        #expect(TextPostProcessor.process("A hermitian") == "A†")
        #expect(TextPostProcessor.process("trace of A") == "tr(A)")
        #expect(TextPostProcessor.process("the trace of B") == "tr(B)")
        #expect(TextPostProcessor.process("rank of A") == "rank(A)")
        #expect(TextPostProcessor.process("the rank of M") == "rank(M)")
        // Guards
        #expect(TextPostProcessor.process("transpose the file") == "transpose the file")
        #expect(TextPostProcessor.process("inverse of the claim") == "inverse of the claim")
        #expect(TextPostProcessor.process("rank of the team") == "rank of the team")
        // Matmul / matrix product
        #expect(TextPostProcessor.process("A matmul B") == "AB")
        #expect(TextPostProcessor.process("a matmul b") == "ab")
        #expect(TextPostProcessor.process("A matrix multiply B") == "AB")
        #expect(TextPostProcessor.process("A matrix times B") == "AB")
        #expect(TextPostProcessor.process("A matrix product B") == "AB")
        #expect(TextPostProcessor.process("matrix product of A and B") == "AB")
        #expect(TextPostProcessor.process("matrix multiply of X and Y") == "XY")
        #expect(TextPostProcessor.process("matrix multiplication of A and B") == "AB")
        // Eigenvalues / eigenvectors / determinant
        #expect(TextPostProcessor.process("eigenvalues of A") == "λ(A)")
        #expect(TextPostProcessor.process("the eigenvalues of B") == "λ(B)")
        #expect(TextPostProcessor.process("eigenvalue of M") == "λ(M)")
        #expect(TextPostProcessor.process("eigenvectors of A") == "V(A)")
        #expect(TextPostProcessor.process("the eigenvectors of B") == "V(B)")
        #expect(TextPostProcessor.process("eigenvector of M") == "V(M)")
        #expect(TextPostProcessor.process("determinant of A") == "det(A)")
        #expect(TextPostProcessor.process("the determinant of B") == "det(B)")
        #expect(TextPostProcessor.process("det of A") == "det(A)")
        // Guards — natural language must not fire
        #expect(TextPostProcessor.process("a times a day") == "a times a day")
        #expect(TextPostProcessor.process("product of A and B") == "product of A and B")
        ...
        // Relations: orthogonal / parallel; products; argmin/argmax
        #expect(TextPostProcessor.process("A orthogonal to B") == "A ⊥ B")
        #expect(TextPostProcessor.process("A is orthogonal to B") == "A ⊥ B")
        #expect(TextPostProcessor.process("u perpendicular to v") == "u ⊥ v")
        #expect(TextPostProcessor.process("A parallel to B") == "A ∥ B")
        #expect(TextPostProcessor.process("A is parallel to B") == "A ∥ B")
        #expect(TextPostProcessor.process("dot product of u and v") == "⟨u, v⟩")
        #expect(TextPostProcessor.process("inner product of a and b") == "⟨a, b⟩")
        #expect(TextPostProcessor.process("the inner product of x and y") == "⟨x, y⟩")
        #expect(TextPostProcessor.process("cross product of u and v") == "u × v")
        #expect(TextPostProcessor.process("the cross product of a and b") == "a × b")
        #expect(TextPostProcessor.process("argmin of f") == "argmin(f)")
        #expect(TextPostProcessor.process("arg min of f") == "argmin(f)")
        #expect(TextPostProcessor.process("argmax of g") == "argmax(g)")
        #expect(TextPostProcessor.process("arg max of h") == "argmax(h)")
        // Guards
        #expect(TextPostProcessor.process("orthogonal to the idea") == "orthogonal to the idea")
        #expect(TextPostProcessor.process("parallel to the road") == "parallel to the road")
        #expect(TextPostProcessor.process("dot product of the day") == "dot product of the day")
        #expect(TextPostProcessor.process("arg min of the cost") == "arg min of the cost")
        // Subscripts
    ...
    func lightITNRelationsAndAccents() {
        ...
        // Quantifiers with variable (single letter only)
        #expect(TextPostProcessor.process("for all x") == "∀x")
        #expect(TextPostProcessor.process("for every n") == "∀n")
        #expect(TextPostProcessor.process("forall x") == "∀x")
        #expect(TextPostProcessor.process("there exists x") == "∃x")
        #expect(TextPostProcessor.process("exists y") == "∃y")
        #expect(TextPostProcessor.process("there exists unique x") == "∃!x")
        #expect(TextPostProcessor.process("unique exists x") == "∃!x")
        // Implies / maps-to
        #expect(TextPostProcessor.process("A implies B") == "A ⇒ B")
        #expect(TextPostProcessor.process("p implies q") == "p ⇒ q")
        #expect(TextPostProcessor.process("A maps to B") == "A ↦ B")
        #expect(TextPostProcessor.process("f maps to g") == "f ↦ g")
        // Sup / inf (not infinity-norm — that is "inf norm of")
        #expect(TextPostProcessor.process("sup of S") == "sup(S)")
        #expect(TextPostProcessor.process("supremum of S") == "sup(S)")
        #expect(TextPostProcessor.process("the supremum of A") == "sup(A)")
        #expect(TextPostProcessor.process("inf of S") == "inf(S)")
        #expect(TextPostProcessor.process("infimum of T") == "inf(T)")
        #expect(TextPostProcessor.process("the infimum of A") == "inf(A)")
        ...
        // Primes (derivatives / feet-inches style marks)
        #expect(TextPostProcessor.process("x prime") == "x′")
        #expect(TextPostProcessor.process("f prime") == "f′")
        #expect(TextPostProcessor.process("prime x") == "x′")
        #expect(TextPostProcessor.process("x double prime") == "x″")
        #expect(TextPostProcessor.process("y double prime") == "y″")
        #expect(TextPostProcessor.process("double prime x") == "x″")
        #expect(TextPostProcessor.process("x triple prime") == "x‴")
        #expect(TextPostProcessor.process("f triple prime") == "f‴")
        // Guards
        ...
        #expect(TextPostProcessor.process("there exists a chance") == "there exists a chance")
        #expect(TextPostProcessor.process("implies that") == "implies that")
        #expect(TextPostProcessor.process("maps to the store") == "maps to the store")
        #expect(TextPostProcessor.process("sup of the list") == "sup of the list")
        #expect(TextPostProcessor.process("inf of the day") == "inf of the day")
        // Subset / superset
        #expect(TextPostProcessor.process("A subset of B") == "A ⊆ B")
        #expect(TextPostProcessor.process("A proper subset of B") == "A ⊂ B")
        #expect(TextPostProcessor.process("A subseteq B") == "A ⊆ B")
        #expect(TextPostProcessor.process("A subset or equal B") == "A ⊆ B")
        #expect(TextPostProcessor.process("A superset of B") == "A ⊇ B")
        #expect(TextPostProcessor.process("A proper superset of B") == "A ⊃ B")
        #expect(TextPostProcessor.process("A superseteq B") == "A ⊇ B")
        // Such that (letter-cued only)
        #expect(TextPostProcessor.process("x such that") == "x s.t.")
        #expect(TextPostProcessor.process("n such that") == "n s.t.")
        // Lim sup / lim inf
        #expect(TextPostProcessor.process("lim sup of a") == "limsup(a)")
        #expect(TextPostProcessor.process("limit superior of a") == "limsup(a)")
        #expect(TextPostProcessor.process("the limit superior of x") == "limsup(x)")
        #expect(TextPostProcessor.process("lim inf of a") == "liminf(a)")
        #expect(TextPostProcessor.process("limit inferior of b") == "liminf(b)")
        #expect(TextPostProcessor.process("the limit inferior of y") == "liminf(y)")
        // Guards
        #expect(TextPostProcessor.process("subset of the team") == "subset of the team")
        #expect(TextPostProcessor.process("such that I can") == "such that I can")
        #expect(TextPostProcessor.process("limit superior of the series") == "limit superior of the series")
        // Topology / set operators
        #expect(TextPostProcessor.process("power set of S") == "𝒫(S)")
        #expect(TextPostProcessor.process("the power set of A") == "𝒫(A)")
        #expect(TextPostProcessor.process("closure of S") == "cl(S)")
        #expect(TextPostProcessor.process("the closure of A") == "cl(A)")
        #expect(TextPostProcessor.process("interior of S") == "int(S)")
        #expect(TextPostProcessor.process("the interior of U") == "int(U)")
        #expect(TextPostProcessor.process("boundary of S") == "∂S")
        #expect(TextPostProcessor.process("the boundary of A") == "∂A")
        #expect(TextPostProcessor.process("complement of S") == "Sᶜ")
        #expect(TextPostProcessor.process("the complement of A") == "Aᶜ")
        #expect(TextPostProcessor.process("cardinality of S") == "|S|")
        #expect(TextPostProcessor.process("the cardinality of A") == "|A|")
        #expect(TextPostProcessor.process("card of S") == "|S|")
        // Guards — orthogonal complement must stay LA path
        #expect(TextPostProcessor.process("orthogonal complement of V") == "V⊥")
        #expect(TextPostProcessor.process("power set of the tools") == "power set of the tools")
        #expect(TextPostProcessor.process("closure of the deal") == "closure of the deal")
        #expect(TextPostProcessor.process("interior of the house") == "interior of the house")
        #expect(TextPostProcessor.process("boundary of the property") == "boundary of the property")
        #expect(TextPostProcessor.process("complement of the claim") == "complement of the claim")
        #expect(TextPostProcessor.process("cardinality of the sample") == "cardinality of the sample")
        // Diameter, convex hull, balls, neighborhood
        #expect(TextPostProcessor.process("diameter of S") == "diam(S)")
        #expect(TextPostProcessor.process("the diameter of A") == "diam(A)")
        #expect(TextPostProcessor.process("diam of S") == "diam(S)")
        #expect(TextPostProcessor.process("convex hull of S") == "conv(S)")
        #expect(TextPostProcessor.process("the convex hull of A") == "conv(A)")
        #expect(TextPostProcessor.process("open ball of radius r around x") == "B(x, r)")
        #expect(TextPostProcessor.process("closed ball of radius r around x") == "B̄(x, r)")
        #expect(TextPostProcessor.process("open ball around x of radius r") == "B(x, r)")
        #expect(TextPostProcessor.process("closed ball centered at x of radius 1") == "B̄(x, 1)")
        #expect(TextPostProcessor.process("ball of radius r around x") == "B(x, r)")
        #expect(TextPostProcessor.process("neighborhood of x") == "N(x)")
        #expect(TextPostProcessor.process("the neighborhood of p") == "N(p)")
        // Guards
        // Note: spoken "pipe" → "|" is a global phrase fix — avoid "pipe" in guards.
        #expect(TextPostProcessor.process("diameter of the circle") == "diameter of the circle")
        #expect(TextPostProcessor.process("convex hull of the argument") == "convex hull of the argument")
        #expect(TextPostProcessor.process("neighborhood of the school") == "neighborhood of the school")
        #expect(TextPostProcessor.process("open ball of radius the") == "open ball of radius the")
        // Distance, angle, morphisms
        #expect(TextPostProcessor.process("distance from x to y") == "d(x, y)")
        #expect(TextPostProcessor.process("the distance from a to b") == "d(a, b)")
        #expect(TextPostProcessor.process("distance between x and y") == "d(x, y)")
        #expect(TextPostProcessor.process("the distance between p and q") == "d(p, q)")
        #expect(TextPostProcessor.process("angle between u and v") == "∠(u, v)")
        #expect(TextPostProcessor.process("the angle between a and b") == "∠(a, b)")
        #expect(TextPostProcessor.process("A isomorphic to B") == "A ≅ B")
        #expect(TextPostProcessor.process("A is isomorphic to B") == "A ≅ B")
        #expect(TextPostProcessor.process("G homeomorphic to H") == "G ≃ H")
        #expect(TextPostProcessor.process("X is homeomorphic to Y") == "X ≃ Y")
        #expect(TextPostProcessor.process("homomorphism from A to B") == "Hom(A, B)")
        #expect(TextPostProcessor.process("the homomorphism from G to H") == "Hom(G, H)")
        // Guards
        #expect(TextPostProcessor.process("distance from the house") == "distance from the house")
        #expect(TextPostProcessor.process("distance between the cities") == "distance between the cities")
        #expect(TextPostProcessor.process("angle between the walls") == "angle between the walls")
        #expect(TextPostProcessor.process("isomorphic to the idea") == "isomorphic to the idea")
        #expect(TextPostProcessor.process("homomorphism from the start") == "homomorphism from the start")
        // Congruence, End/Aut, injects/surjects
        #expect(TextPostProcessor.process("a congruent to b mod n") == "a ≡ b (mod n)")
        #expect(TextPostProcessor.process("A congruent to B modulo m") == "A ≡ B (mod m)")
        #expect(TextPostProcessor.process("x is congruent to y mod 3") == "x ≡ y (mod 3)")
        #expect(TextPostProcessor.process("endomorphism of A") == "End(A)")
        #expect(TextPostProcessor.process("the endomorphism of G") == "End(G)")
        #expect(TextPostProcessor.process("automorphism of A") == "Aut(A)")
        #expect(TextPostProcessor.process("the automorphism of G") == "Aut(G)")
        #expect(TextPostProcessor.process("A injects into B") == "A ↪ B")
        #expect(TextPostProcessor.process("A injects to B") == "A ↪ B")
        #expect(TextPostProcessor.process("A surjects onto B") == "A ↠ B")
        #expect(TextPostProcessor.process("A surjects to B") == "A ↠ B")
        #expect(TextPostProcessor.process("A bijects to B") == "A ⤖ B")
        #expect(TextPostProcessor.process("A bijects onto B") == "A ⤖ B")
        // Guards
        #expect(TextPostProcessor.process("congruent to the claim") == "congruent to the claim")
        #expect(TextPostProcessor.process("endomorphism of the group") == "endomorphism of the group")
        #expect(TextPostProcessor.process("injects into the market") == "injects into the market")
        // Image / cokernel / coimage / divides
        #expect(TextPostProcessor.process("image of f") == "im(f)")
        #expect(TextPostProcessor.process("the image of g") == "im(g)")
        #expect(TextPostProcessor.process("im of f") == "im(f)")
        #expect(TextPostProcessor.process("cokernel of f") == "coker(f)")
        #expect(TextPostProcessor.process("the cokernel of g") == "coker(g)")
        #expect(TextPostProcessor.process("coker of f") == "coker(f)")
        #expect(TextPostProcessor.process("coimage of f") == "coim(f)")
        #expect(TextPostProcessor.process("the coimage of g") == "coim(g)")
        #expect(TextPostProcessor.process("A divides B") == "A ∣ B")
        #expect(TextPostProcessor.process("a divides b") == "a ∣ b")
        #expect(TextPostProcessor.process("A does not divide B") == "A ∤ B")
        #expect(TextPostProcessor.process("a does not divide b") == "a ∤ b")
        // Guards
        #expect(TextPostProcessor.process("image of the photo") == "image of the photo")
        #expect(TextPostProcessor.process("cokernel of the process") == "cokernel of the process")
        #expect(TextPostProcessor.process("divides the room") == "divides the room")
        #expect(TextPostProcessor.process("does not divide the class") == "does not divide the class")
        // Number theory / group order
        #expect(TextPostProcessor.process("A coprime to B") == "gcd(A, B) = 1")
        #expect(TextPostProcessor.process("A is coprime to B") == "gcd(A, B) = 1")
        #expect(TextPostProcessor.process("a relatively prime to b") == "gcd(a, b) = 1")
        #expect(TextPostProcessor.process("order of g") == "ord(g)")
        #expect(TextPostProcessor.process("the order of g") == "ord(g)")
        #expect(TextPostProcessor.process("ord of g") == "ord(g)")
        #expect(TextPostProcessor.process("index of H in G") == "[G:H]")
        #expect(TextPostProcessor.process("the index of H in G") == "[G:H]")
        #expect(TextPostProcessor.process("phi of n") == "φ(n)")
        #expect(TextPostProcessor.process("euler totient of n") == "φ(n)")
        #expect(TextPostProcessor.process("the euler phi of n") == "φ(n)")
        #expect(TextPostProcessor.process("gcd of a and b") == "gcd(a, b)")
        #expect(TextPostProcessor.process("lcm of a and b") == "lcm(a, b)")
        // Guards
        #expect(TextPostProcessor.process("coprime to the issue") == "coprime to the issue")
        #expect(TextPostProcessor.process("order of the day") == "order of the day")
        #expect(TextPostProcessor.process("index of the book") == "index of the book")
        #expect(TextPostProcessor.process("phi of the system") == "phi of the system")
        // Group theory: center, centralizer, normalizer, commutator
        #expect(TextPostProcessor.process("center of G") == "Z(G)")
        #expect(TextPostProcessor.process("the center of G") == "Z(G)")
        #expect(TextPostProcessor.process("centre of G") == "Z(G)")
        #expect(TextPostProcessor.process("centralizer of H in G") == "C_G(H)")
        #expect(TextPostProcessor.process("the centralizer of H in G") == "C_G(H)")
        #expect(TextPostProcessor.process("normalizer of H in G") == "N_G(H)")
        #expect(TextPostProcessor.process("the normalizer of H in G") == "N_G(H)")
        #expect(TextPostProcessor.process("commutator of a and b") == "[a, b]")
        #expect(TextPostProcessor.process("the commutator of x and y") == "[x, y]")
        #expect(TextPostProcessor.process("derived subgroup of G") == "G'")
        #expect(TextPostProcessor.process("the derived subgroup of G") == "G'")
        #expect(TextPostProcessor.process("commutator subgroup of G") == "G'")
        // Guards
        #expect(TextPostProcessor.process("center of the city") == "center of the city")
        #expect(TextPostProcessor.process("centre of the room") == "centre of the room")
        #expect(TextPostProcessor.process("normalizer of the data") == "normalizer of the data")
        #expect(TextPostProcessor.process("commutator of the sentence") == "commutator of the sentence")
        // Group actions: stab, orbit, acts on, Fix
        #expect(TextPostProcessor.process("stabilizer of x") == "Stab(x)")
        #expect(TextPostProcessor.process("the stabilizer of x") == "Stab(x)")
        #expect(TextPostProcessor.process("stab of x") == "Stab(x)")
        #expect(TextPostProcessor.process("orbit of x") == "Orb(x)")
        #expect(TextPostProcessor.process("the orbit of x") == "Orb(x)")
        #expect(TextPostProcessor.process("orb of x") == "Orb(x)")
        #expect(TextPostProcessor.process("G acts on X") == "G ↷ X")
        #expect(TextPostProcessor.process("G act on X") == "G ↷ X")
        #expect(TextPostProcessor.process("fixed points of g") == "Fix(g)")
        #expect(TextPostProcessor.process("the fixed points of g") == "Fix(g)")
        #expect(TextPostProcessor.process("fixed point of g") == "Fix(g)")
        // Guards
        #expect(TextPostProcessor.process("stabilizer of the patient") == "stabilizer of the patient")
        #expect(TextPostProcessor.process("orbit of the moon") == "orbit of the moon")
        #expect(TextPostProcessor.process("acts on the stage") == "acts on the stage")
        #expect(TextPostProcessor.process("fixed points of the argument") == "fixed points of the argument")
        // Cosets, quotients, (semi)direct products
        #expect(TextPostProcessor.process("G mod H") == "G/H")
        #expect(TextPostProcessor.process("G modulo H") == "G/H")
        #expect(TextPostProcessor.process("G over H") == "G/H")
        #expect(TextPostProcessor.process("quotient of G by H") == "G/H")
        #expect(TextPostProcessor.process("the quotient of G by H") == "G/H")
        #expect(TextPostProcessor.process("left cosets of H in G") == "G/H")
        #expect(TextPostProcessor.process("right cosets of H in G") == #"H\G"#)
        #expect(TextPostProcessor.process("cosets of H in G") == "G/H")
        #expect(TextPostProcessor.process("A semidirect product B") == "A ⋊ B")
        #expect(TextPostProcessor.process("A semidirect B") == "A ⋊ B")
        #expect(TextPostProcessor.process("semidirect product of A and B") == "A ⋊ B")
        #expect(TextPostProcessor.process("A rtimes B") == "A ⋊ B")
        #expect(TextPostProcessor.process("A ltimes B") == "A ⋉ B")
        #expect(TextPostProcessor.process("A direct product B") == "A × B")
        #expect(TextPostProcessor.process("direct product of A and B") == "A × B")
        // Guards
        #expect(TextPostProcessor.process("mod the rules") == "mod the rules")
        #expect(TextPostProcessor.process("quotient of the day") == "quotient of the day")
        #expect(TextPostProcessor.process("cosets of the team") == "cosets of the team")
        #expect(TextPostProcessor.process("semidirect product of money") == "semidirect product of money")
        #expect(TextPostProcessor.process("direct product of labor") == "direct product of labor")
        #expect(TextPostProcessor.process("hat check") == "hat check")
        #expect(TextPostProcessor.process("bar exam") == "bar exam")
        #expect(TextPostProcessor.process("prime time") == "prime time")
        #expect(TextPostProcessor.process("prime number") == "prime number")
        #expect(TextPostProcessor.process("double prime rib") == "double prime rib")
    }
