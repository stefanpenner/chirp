...
struct DictationDictionaryTests {
    ...
    func builtIn() {
        withCleanOverrides {
            ...
            #expect(DictationDictionary.apply("message on linked in") == "message on LinkedIn")
            #expect(DictationDictionary.apply("post on face book") == "post on Facebook")
            #expect(DictationDictionary.apply("ping on whats app") == "ping on WhatsApp")
            #expect(DictationDictionary.apply("open tik tok") == "open TikTok")
            #expect(DictationDictionary.apply("check insta gram") == "check Instagram")
            #expect(DictationDictionary.apply("send snap chat") == "send Snapchat")
            #expect(DictationDictionary.apply("sync drop box") == "sync Dropbox")
            #expect(DictationDictionary.apply("edit word press") == "edit WordPress")
            #expect(DictationDictionary.apply("login sales force") == "login Salesforce")
            #expect(DictationDictionary.apply("install micro soft office") == "install Microsoft office")
            #expect(DictationDictionary.apply("learn dev ops") == "learn DevOps")
            #expect(DictationDictionary.apply("ship git ops") == "ship GitOps")
            #expect(DictationDictionary.apply("front end and back end") == "frontend and backend")
            #expect(DictationDictionary.apply("search stack overflow") == "search Stack Overflow")
            #expect(DictationDictionary.apply("deploy terra form") == "deploy Terraform")
            #expect(DictationDictionary.apply("open post man") == "open Postman")
            #expect(DictationDictionary.apply("design in fig ma") == "design in Figma")
            #expect(DictationDictionary.apply("notes in no tion") == "notes in Notion")
            #expect(DictationDictionary.apply("chat on dis cord") == "chat on Discord")
            #expect(DictationDictionary.apply("write c sharp") == "write C#")
            #expect(DictationDictionary.apply("write c plus plus") == "write C++")
            #expect(DictationDictionary.apply("legacy objective c") == "legacy Objective-C")
            #expect(DictationDictionary.apply("use core data") == "use Core Data")
            #expect(DictationDictionary.apply("open git hub actions") == "open GitHub Actions")
            #expect(DictationDictionary.apply("edit the read me") == "edit the README")
            #expect(DictationDictionary.apply("bucket on s three") == "bucket on S3")
            #expect(DictationDictionary.apply("my mac book pro") == "my MacBook Pro")
    ...
    func viaPostProcessor() {
        withCleanOverrides {
            ...
            #expect(TextPostProcessor.process("deploy terra form please") == "deploy Terraform please")
        }
    }
