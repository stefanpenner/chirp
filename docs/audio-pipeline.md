...
- **Command soak** — Dragon phrases TTS → parse hit rate ≥ 80% (`CommandPhraseEval.soakMinHitRate`)
...
(e.g. Capta → cap that) before parse. Live soak: `AudioCorpusPipelineTests`
`commandPhraseSoak` (TTS → ASR → parse; budget ≥ 80% hit rate).
...
- **tab N times** / **press tab 3 times** — insert N tabs into session (`TabN.tla`)
- **press enter N times** / **enter twice** — insert N returns into session (`EnterN.tla`)
- **press space N times** / **space twice** — insert N spaces into session (`SpaceN.tla`)
...
- **select up N sentences** / **select down 2 sentences** — select N sentences from caret (`SelectSentencesN.tla`)
- **move up N sentences** / **move down 3 sentences** — jump to sentence start N steps (`MoveSentencesN.tla`)
- **select again** / **select next occurrence** — walk prior/next match of last select X / select that / select last word / select last sentence|paragraph|line (`SelectAgain.tla`). ASR near-misses: select a gain, selected that, scratch hat / scrap hat
...
- **go to X** / **move to X** / **jump to X** — move caret to start of last occurrence of X; next content **inserts** at that offset (session caret dual; does not replace the phrase). `GoToPhrase.tla` / `SessionCaretDecision`
- **go after X** / **move after X** — move caret to end of last occurrence of X; next content inserts after it. Does **not** steal **go to start** / **go to next sentence**
...
- **move left** / **previous word** — cursor left one word (⌥←); sets `sessionCaret` so next content inserts mid-buffer (`WordCaret.tla`)
- **move right** / **next word** — cursor right one word (⌥→); sets `sessionCaret` (end → append mode)
- **move left N words** / **back N words** / **move previous N words** — cursor left N words (⌥← × N) + session caret. Does **not** steal bare **move left**
- **move right N words** / **forward N words** / **move next N words** — cursor right N words (⌥→ × N) + session caret. Does **not** steal bare **move right**
- **move left N characters** / **back N characters** — cursor left N characters (← × N) + `sessionCaret` for mid-buffer insert (`CharacterCaret.tla`)
- **move right N characters** / **forward N characters** — cursor right N characters (→ × N) + `sessionCaret`. Dual: `MoveN.tla` / `CharacterCaret.tla`
