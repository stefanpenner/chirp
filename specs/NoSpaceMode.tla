---- MODULE NoSpaceMode ----
(*
  Sticky no-space mode for dictation commits.

  Modes: off | on
  SetOn / SetOff switch sticky mode (commands do not emit text).
  Commit records that text is emitted under the current mode
  (when on, SegmentJoiner uses empty separator between segments;
   does not pack letters — that is SpellMode).
  Reset starts a new hold-to-talk session (mode → off).

  Dual of AppState.noSpaceMode + DictationCommand.setNoSpaceMode.
*)
...
\* New hold-to-talk session / cancel
Reset ==
  /\ mode' = "off"
  /\ commits' = 0
  /\ lastOp' = "reset"
